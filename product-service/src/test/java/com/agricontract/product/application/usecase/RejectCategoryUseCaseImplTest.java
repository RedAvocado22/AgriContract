...
class RejectCategoryUseCaseImplTest {

    @Mock private CategoryRepository categoryRepository;

    private RejectCategoryUseCase useCase;

    @Test
    void execute_pendingCategory_rejectsWithReasonAndSaves() {
        useCase = new RejectCategoryUseCaseImpl(categoryRepository);
        ...
        CategoryResponse response = useCase.execute("category-1", "trung ten");

        assertThat(response.status()).isEqualTo(CategoryStatus.REJECTED);
        assertThat(response.rejectionReason()).isEqualTo("trung ten");
    ...
    void execute_notFound_throws() {
        ...
        assertThatThrownBy(() -> useCase.execute("missing", "reason")).isInstanceOf(CategoryNotFoundException.class);

        verify(categoryRepository, never()).save(any());
    }

    @Test
    void execute_blankReason_throwsIllegalArgument() {
        useCase = new RejectCategoryUseCaseImpl(categoryRepository);
        Category category = Category.propose(new CategoryId("category-1"), "Gao", "gao", "seller-1", "seller@test.com");
        when(categoryRepository.findById(new CategoryId("category-1"))).thenReturn(Optional.of(category));

        assertThatThrownBy(() -> useCase.execute("category-1", " ")).isInstanceOf(IllegalArgumentException.class);
