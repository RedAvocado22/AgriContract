...
| `geoRiskLevel` | Enum, nullable (`LOW_RISK`, `HIGH_RISK`) | **Mới (06/07/2026).** Kết quả thật của NDVI check — chỉ có giá trị khi `geoVerificationStatus = CHECKED`. `NULL` khi `PENDING`/`UNVERIFIED` — xem §2.2.4. |
...
### 2.2.4 Geo-risk verification qua vệ tinh — Sentinel-2 NDVI (mới, 06/07/2026)
...
**Cơ chế:** ngay sau khi `PlotRegistryEntry` được tạo/update (qua `ImportPlotsFromKML`/`AddPlotManually`), hệ thống tự động (async, không chặn response cho seller) gọi Sentinel Hub Statistical API (Copernicus Data Space Ecosystem) hỏi giá trị NDVI tại toạ độ/polygon đó, dùng ảnh gần nhất mốc 31/12/2020 — dữ liệu Sentinel-2 đã có sẵn trong archive từ 2015 (vệ tinh đầu phóng tháng 6/2015), không phải chờ vệ tinh bay qua chụp mới. NDVI dưới ngưỡng rừng (giá trị cụ thể chưa chốt, cần thực nghiệm) → `geoRiskLevel = HIGH_RISK`; trên ngưỡng → `LOW_RISK`.
...
**Timeout — `geoVerificationTimeoutHours`** (đề xuất 4-6 giờ, `application.yml`): vì dữ liệu Sentinel-2 tại mốc 31/12/2020 đã tồn tại sẵn trong archive (không phải giám sát real-time), timeout ở đây chỉ để hứng lỗi tạm thời (mây che phải ghép nhiều ảnh lân cận, rate limit, lỗi mạng) — tính bằng **giờ**, khác hẳn các timeout khác trong hệ thống dựa trên hành vi người (`sellerResponseWindowDays`...) vốn tính bằng ngày. Hết `geoVerificationTimeoutHours` mà API chưa trả kết quả → `geoVerificationStatus = UNVERIFIED` (khác `HIGH_RISK` — đây là "chưa biết", không phải "có bằng chứng rủi ro").
...
- Ngưỡng NDVI cụ thể phân biệt rừng/không rừng **chưa chốt** — cần thực nghiệm với dữ liệu thật trước khi code.

**Điểm còn treo, chưa chốt (06/07/2026):** cơ chế bổ sung bằng ảnh EXIF (bắt buộc chụp ảnh tại chỗ qua app lúc đăng ký plot, đối chiếu GPS trong EXIF với toạ độ khai) đã thống nhất về nguyên tắc — lớp phòng thủ yếu, chi phí thấp, đáng làm, giữ nguyên không cần nâng cấp gì thêm (không giải quyết được GPS spoofing, chỉ lọc case sơ hở) — nhưng field/flow cụ thể (tên field lưu trên `PlotRegistryEntry`, ngưỡng khoảng cách đối chiếu chấp nhận được, UI camera capture) **chưa được spec trong session này**, cần 1 session riêng trước khi đưa vào migration.
...
- **`CreateListing`:** đổi input — nhận `List<registryEntryId>` (chọn từ danh sách plot đã có sẵn trong registry của seller, UI dạng checkbox) thay vì `List<ProductPlotRequest>` với `geoJson` đầy đủ. Use case tự copy dữ liệu từ `PlotRegistryEntry` đã chọn sang `ProductPlot` mới (snapshot, §2.4). Validate: `registryEntryId` phải thuộc đúng `sellerId` đang tạo listing, ít nhất 1 plot được chọn (giữ nguyên yêu cầu ≥1 plot từ bản gốc).
  - **Validate mới (06/07/2026):** reject nếu bất kỳ `registryEntryId` nào đang `geoVerificationStatus = PENDING` (§2.2.4) — tránh race condition listing lên sàn trước khi biết kết quả satellite check. Không áp dụng cho `CHECKED` (dù `HIGH_RISK`) hay `UNVERIFIED` — cả hai đều được coi là "đã có câu trả lời" (dù câu trả lời là rủi ro cao hay không xác minh được), chỉ `PENDING` (chưa có câu trả lời gì) mới bị chặn.
...
- **Geo-risk verification qua vệ tinh — Sentinel-2 NDVI (mới, 06/07/2026, xem §2.2.4):** lớp bổ sung độc lập, nhắm đúng câu hỏi "đất có phải rừng tính đến 31/12/2020 không" mà JTS/province cross-check không trả lời được. Không chặn giao dịch — chỉ dán nhãn `HIGH_RISK`/`LOW_RISK` cho buyer tự quyết định. NDVI 10m + ngưỡng nhị phân là proxy thô, không phải xác minh EUDR pháp lý đầy đủ — xem giới hạn chi tiết ở §2.2.4.
...
- **EXIF photo cross-check (mới, 06/07/2026, đề xuất trong session §2.2.4)** — thống nhất về nguyên tắc (lớp phòng thủ yếu, chi phí thấp, đáng làm) nhưng field/flow cụ thể chưa được spec trong session này — để dành session riêng, không phải bị bỏ quên.
...
- **`geoVerificationStatus`/`geoRiskLevel` (mới)** — tách trạng thái tiến trình check (`PENDING`/`CHECKED`/`UNVERIFIED`) khỏi kết quả rủi ro thật (`LOW_RISK`/`HIGH_RISK`), 2 field riêng biệt để tránh gộp nhầm "chưa biết" với "có bằng chứng rủi ro" (§2.1, §2.2.4).
- **Sentinel Hub NDVI check (Copernicus Data Space Ecosystem)** — async, chạy 1 lần/`PlotRegistryEntry` ngay sau tạo/update, không lặp lại mỗi lần tạo listing. Dữ liệu Sentinel-2 miễn phí, không giới hạn thương mại; chi phí Processing Unit không đáng kể với diện tích plot nông hộ. Không chặn giao dịch — chỉ dán nhãn cho buyer tự quyết định (§2.2.4).
- **`geoVerificationTimeoutHours`** (đề xuất 4-6 giờ) — timeout tính bằng giờ chứ không phải ngày, vì dữ liệu Sentinel-2 tại mốc 31/12/2020 đã có sẵn trong archive (từ 2015), timeout chỉ hứng lỗi tạm thời (mây che, rate limit), không phải chờ dữ liệu tồn tại (§2.2.4).
- **`CreateListing` chặn khi `PENDING`** — tránh race condition listing lên sàn trước khi có kết quả check (§4).

**Chưa đóng — 3 điểm còn treo (06/07/2026):** (1) ngưỡng NDVI phân biệt rừng/không rừng chưa chốt, cần thực nghiệm; (2) `geoVerificationTimeoutHours` là placeholder 4-6h, chưa vận hành thật để kiểm chứng; (3) EXIF photo cross-check mới thống nhất nguyên tắc, chưa spec field/flow (§6). Xem chi tiết đầy đủ ở §2.2.4.

Product Phase 2 (Farm Plot Geolocation) — **đóng session, sẵn sàng đưa vào Architecture/SDS/TechnicalSpec chính thức**, ngoại trừ bổ sung 06/07/2026 (geo-risk verification qua vệ tinh, xem ngay trên) — phần này **chưa đóng**, còn 3 điểm treo cần giải quyết trước khi coi là chốt cứng.
...
*Design session: 02/07/2026 · Cập nhật: 04/07/2026 (GEOMETRY/JTS validation, Plot Registry + KML import) · 05/07/2026 (file-service dependency, province cross-check mitigation) · 05/07/2026 (thêm `varietyName`, cross-reference `pricing-service`) · 06/07/2026 (geo-risk verification qua vệ tinh Sentinel-2 NDVI, `geoVerificationStatus`/`geoRiskLevel`, §2.2.4 — chưa đóng, còn 3 điểm treo) · Chưa code · Chưa đưa vào Architecture/SDS/TechnicalSpec chính thức.*
