...
push(table(
  ...
  [
    ...
    ["FlagSuspiciousPatternUseCase", "Tính composite fraud score theo cặp/account → publish event hold khi vượt ngưỡng"],
  ],
  { size: 18 }
));
push(codeblock([
...
push(bullet([runs("AML phát hiện giao dịch KẾ TIẾP, không phải hiện tại. ", { bold: true }), runs("Detect dựa trên pattern lặp lại cần đủ lịch sử — chính giao dịch làm score vượt ngưỡng không bị hold, chỉ giao dịch tiếp theo của cặp đó. Giới hạn cấu trúc, không sửa bằng đổi threshold.", {})]));
