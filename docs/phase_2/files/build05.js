...
push(P("Hai aggregate root độc lập trong cùng dịch vụ/DB: Contract và Milestone. Milestone KHÔNG phải entity con của Contract — milestone thứ 3 quyết toán không cần atomic cùng lúc với milestone khác, nên tách aggregate để mỗi milestone có transaction riêng (Effective Aggregate Design — Vernon). Signature BUYER/SELLER và ContractTerms là value object trong Contract. OTP challenge được lưu riêng trong signature_otp; chữ ký INSPECTOR thuộc inspection-service/inspection_db, không ghi chéo contract_db."));
push(table(
  [2400, 2200, 5038],
  ...
  [
    ...
    ["Signature", "Value object (trong Contract)", "Bản ghi ký BUYER/SELLER, bind authTime + signedContentHash; UNIQUE(contractId, signerRole)"],
  ],
  { size: 18 }
));
...
push(table(
  ...
  [
    ...
    ["SIGNED", "Admin MarkActivationFailed sau khi hết retry; có leg đã LOCKED", "ACTIVATION_REFUND_PENDING", "Phát refund cho từng leg đã khoá; không penalty/reputation"],
    ["ACTIVATION_REFUND_PENDING", "Mọi bank.refund_completed đã xác nhận", "ACTIVATION_FAILED", "Terminal invariant: tổng lock cấp hợp đồng = 0"],
    ["SIGNED", "Admin MarkActivationFailed; không leg nào đã LOCKED", "ACTIVATION_FAILED", "Kết thúc kỹ thuật, không rollback chữ ký"],
...
push(P([runs("SIGNED ≠ mốc tiền, ACTIVE mới là mốc tiền: ", { bold: true }), runs("SIGNED được tạo khi đủ hai chữ ký hợp lệ và contract.signed publish ngay. ACTIVE chỉ đạt khi mọi leg cọc cần thiết đã bank.lock_completed và escrow.deposit_locked. Nếu một leg fail, escrow retry tối đa 3 lần theo backoff 5 phút/30 phút/2 giờ, giữ nguyên sourceEventId của leg. Hết retry publish escrow.deposit_lock_failed với trạng thái từng leg; Contract vẫn SIGNED. ADMIN chọn RetryDepositLock hoặc MarkActivationFailed. Nếu đã có partial lock, Contract vào ACTIVATION_REFUND_PENDING, refund từng leg đã khoá và chỉ sang ACTIVATION_FAILED sau mọi confirmation; refund fail thì đứng lại và alert. Không rollback chữ ký, không penalty/reputation vì đây là lỗi kích hoạt kỹ thuật. Analytics phải phân biệt contract.signed với hợp đồng đã vận hành thật.", {})]));
...
uc("UC-C1", "Ký hợp đồng (2 bước: InitiateSign + VerifyOtpAndSign)", [
  ...
  ["Luồng chính", "1) InitiateSign tính signedContentHash hiện tại, persist challenge {otpId, contractId, signerRole, userId, authTime, signedContentHash, HMAC-SHA256(OTP, serverPepper), expiry, attempts}; gọi sync OTP email với otpId + hash. 2) VerifyOtpAndSign load đúng otpId, bind JWT sub/contract/role/hash hiện tại, kiểm expiry/attempt/HMAC; tạo Signature từ challenge. Từ chữ ký đầu tiên đóng băng ContractTerms; chữ ký thứ hai phải cùng signedContentHash. Đủ BUYER+SELLER → SIGNED, publish notification.contract_anchor_requested và contract.signed(signedContentHash, deposit amounts) cho escrow/analytics/audit"],
  ["Ngoại lệ", "Thẩm quyền hết hạn/session cũ, otpId đã dùng/hết hạn/quá attempts, user-contract-role không khớp hoặc terms đổi sau khi phát OTP → REJECT; terms đã có chữ ký mà bị sửa → 409"],
...
push(codeblock([
  ...
  "  listing_id          CHAR(36) NOT NULL,          -- ref product-service (plain UUID)",
  "  buyer_id            CHAR(36) NOT NULL,          -- ref user-service",
  "  seller_id           CHAR(36) NOT NULL,          -- ref user-service",
  "  status              VARCHAR(28) NOT NULL,   -- OFFERED..ACTIVE/ACTIVATION_REFUND_PENDING/ACTIVATION_FAILED/SETTLED/CANCELLED",
  ...
  "  contract_id                  CHAR(36) PRIMARY KEY REFERENCES contract(contract_id),",
  ...
  "  seller_deposit_rate          DECIMAL(5,4) NOT NULL DEFAULT 0,",
  "  dispute_floor_release_rate   DECIMAL(5,4) NOT NULL DEFAULT 0.50,",
  "  force_majeure_report_window_days INT NOT NULL DEFAULT 3",
  ");",
  "",
  "CREATE TABLE milestone_term (           -- lịch giao hàng bất biến (snapshot)",
  "  term_id             CHAR(36) PRIMARY KEY,",
  "  contract_id         CHAR(36) NOT NULL REFERENCES contract(contract_id),",
  ...
  "  seller_evidence_file_id CHAR(36) NULL,            -- ref file-service",
  "  buyer_received_weight  DECIMAL(15,2) NULL,",
  "  buyer_evidence_file_id CHAR(36) NULL,",
  "  force_majeure_claim_id CHAR(36) NULL,",
...
push(codeblock([
  ...
  "  signer_user_id      CHAR(36) NOT NULL,         -- = JWT sub",
  ...
  "  signed_content_hash VARCHAR(64) NOT NULL,",
  "  otp_hash     VARCHAR(64) NOT NULL, -- HMAC-SHA256(OTP, serverPepper), không plaintext",
  "  sent_at      TIMESTAMP(3) NOT NULL, expires_at TIMESTAMP(3) NOT NULL,",
  "  verified_at  TIMESTAMP(3) NULL, attempt_count INT NOT NULL DEFAULT 0",
  ");",
  "",
  "CREATE TABLE milestone_sync_outbox (    -- Local Outbox (§2.6)",
  "  outbox_id CHAR(36) PRIMARY KEY, milestone_id CHAR(36) NOT NULL, contract_id CHAR(36) NOT NULL,",
  ...
  "  event_id CHAR(36) PRIMARY KEY, aggregate_id CHAR(36) NOT NULL, routing_key VARCHAR(60) NOT NULL,",
...
push(table(
  ...
  [
    ...
    ["EscrowMilestone", "Entity (per milestone)", "Trạng thái batch: LOCKED / PROVISIONALLY_RELEASED / RELEASED / REFUNDED_PARTIAL / PENALIZED; bufferAmount là snapshot phán quyết, không phải số dư"],
  ],
  { size: 18 }
));
...
uc("UC-E1", "Khoá cọc cấp hợp đồng khi SIGNED", [
  ["Trigger", "Nhận contract.signed — payload mang sẵn buyerDepositAmount/sellerDepositAmount (đã tính = rate × totalAmount ở contract-service). escrow-service KHÔNG tự nhân rate×totalAmount: là event consumer, không Feign ngược lấy ContractTerms, nên payload phải mang amount đã tính"],
...
push(P("Toàn bộ đợi confirmation event mới đổi state. Với lock cọc kích hoạt, bank.lock_failed được retry per-leg tối đa 3 lần (5m/30m/2h), cùng sourceEventId; hết retry publish escrow.deposit_lock_failed. Với release/seize/refund, failure giữ state hiện tại, alert và retry idempotent; không tự coi là thành công."));
...
push(codeblock([
  ...
  "  escrow_account_id CHAR(36) PRIMARY KEY,",
  ...
  "  escrow_milestone_id CHAR(36) PRIMARY KEY,",
  "  escrow_account_id   CHAR(36) NOT NULL REFERENCES escrow_account(escrow_account_id),",
  "  milestone_id        CHAR(36) NOT NULL,        -- ref contract-service (plain UUID)",
  "  milestone_index     INT NOT NULL,",
  "  status              VARCHAR(28) NOT NULL, -- LOCKED/PROVISIONALLY_RELEASED/RELEASED/REFUNDED_PARTIAL/PENALIZED",
  "  buffer_amount       DECIMAL(15,2) NULL, -- immutable decision snapshot, không phải balance",
  "  version             BIGINT NOT NULL DEFAULT 0",
  ");",
]));
...
push(P("Giữ tiền hợp pháp (mock) — mô hình FBO/Omnibus: toàn bộ quyền sở hữu theo LedgerEntry append-only. Số dư derive bằng SUM; escrow-service là actor duy nhất phát lệnh tiền. DB principal runtime của bank-service chỉ INSERT + SELECT trên ledger_entry, không UPDATE/DELETE; mọi sửa sai bằng reversal entry có attribution."));
...
push(codeblock([
  ...
  "  source_event_id CHAR(36) NOT NULL UNIQUE,   -- idempotency key (§4.2)",
  "  contract_id     CHAR(36) NOT NULL,",
  "  milestone_id    CHAR(36) NULL,              -- NULL = buyerDepositRate (cấp contract)",
  "  user_id         CHAR(36) NOT NULL,",
  ...
  "-- DB principal: INSERT + SELECT only; không UPDATE/DELETE ledger_entry.",
...
push(P("Structuring là nghĩa vụ riêng: analytics-service nhìn cửa sổ nhiều hợp đồng và publish analytics.structuring_pattern_detected; bank-service tạo SuspiciousTransactionReport append-only, publish bank.suspicious_report_created cho audit-service với sourceType STRUCTURING_REPORT. Không hold giao dịch, không lộ signal/report cho subject để tránh tipping-off."));
push(codeblock([
  "CREATE TABLE suspicious_transaction_report (",
  "  report_id CHAR(36) PRIMARY KEY, buyer_id CHAR(36) NOT NULL, seller_id CHAR(36) NOT NULL,",
  "  contract_ids JSON NOT NULL, near_threshold_count INT NOT NULL,",
  "  window_start TIMESTAMP(3) NOT NULL, window_end TIMESTAMP(3) NOT NULL,",
  "  report_hash VARCHAR(64) NOT NULL, created_at TIMESTAMP(3) NOT NULL",
  "); -- append-only; DB principal không UPDATE/DELETE",
]));
...
push(codeblock([
  ...
  "  verifier_org_id CHAR(36) NOT NULL,          -- External Verifier (generic, không hardcode tên)",
...
push(H2("4.6 Ledger ↔ audit reconciliation và statement export"));
push(P("LedgerAuditReconciliationJob chạy hằng ngày sau cửa sổ anchor, đọc audit_record qua internal read-only API và so RELEASE/SEIZE theo milestone với settledAmount/seizedAmount đã anchor. Mismatch chỉ publish notification.reconciliation_mismatch_requested cho ADMIN + metric; tuyệt đối không auto-fix và không block dòng tiền. Audit-service unavailable cũng alert vận hành."));
push(P("Buyer/Seller tải CSV/PDF qua GET /api/v1/escrows/statements?from&to ở escrow-service; service kiểm participant ownership rồi gọi GET /internal/v1/bank/ledger?participantId&from&to. Bank vẫn không có external route. Bảng kê phản ánh ledger và chỉ là chứng từ giao dịch nền tảng, không phải hoá đơn hay dịch vụ kế toán."));
...
push(table(
  ...
  [
    ...
    ["5", "contract-service", "Đủ 2 chữ ký cùng hash → SIGNED; publish contract.signed(signedContentHash) + anchor request", "audit-service ingest event; email giữ cùng sourceHash"],
...
push(P("Nếu khoá cọc fail: retry per-leg 3 lần; hết retry → escrow.deposit_lock_failed. RetryDepositLock tiếp tục leg còn thiếu. MarkActivationFailed refund mọi leg đã khoá, Contract qua ACTIVATION_REFUND_PENDING và chỉ terminal ACTIVATION_FAILED khi toàn bộ refund confirm; failure đứng lại + alert, tổng lock terminal phải bằng 0."));
...
push(table(
  ...
  [
    ...
    ["depositLockRetryMaxAttempts / backoff", "application.yml", "3 lần; 5 phút / 30 phút / 2 giờ — cùng sourceEventId mỗi leg"],
    ["disputeOptOutWindowDays", "application.yml", "Mặc định 2 ngày làm việc — im lặng dùng mức sàn"],
    ...
    ["disputeFloorReleaseRate", "ContractTerms", "Mặc định 50%; guardrail [50%, 1-level2SafetyBufferRate]"],
...
Packer.toBuffer(doc).then(buf => { writeDocx("/tmp/AgriContract_05_SDS_Part2_v1.docx", buf); });
