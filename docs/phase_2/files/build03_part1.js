...
push(bullet([runs("Lớp hạ tầng dùng chung (infrastructure): ", { bold: true }), runs("Keycloak (IAM), RabbitMQ (message broker), MinIO (object storage), Redis (cache/pub-sub), outbound email adapter (SMTP/SendGrid; MailHog local) và mailbox intake@ do file-service poll qua IMAP.", {})]));
...
push(svcTable([
  ...
  ["Aggregate", "UserProfile (Keycloak sub, organizationName, role, verificationStatus, authorizationExpiresAt, lockedUntil, lastLockRevision, KYC audit metadata verifiedByActorId)"],
  ["Vai trò Keycloak", "SELLER, BUYER, ADMIN, OPERATOR, INSPECTOR"],
]));
...
push(P("Lock projection dùng lockRevision tăng dần per-user: chỉ apply event có revision lớn hơn lastLockRevision; unlock set effectiveLockedUntil do reputation-service tính lại từ mọi lock còn hiệu lực, không clear mù. CreateListing, CreateOffer và sign() đều fail-closed nếu không xác định được KYC/lock state."));
...
push(P("Điểm thiết kế cốt lõi là geolocation cho EUDR. Vì EUDR cấm mass balance, bằng chứng nguồn gốc phải khai đủ toàn bộ mảnh đất của từng hộ đóng góp vào lô hàng — nên geolocation là một mảng plot, không phải một cặp toạ độ đơn. Khai báo đất được tách khỏi việc tạo listing: mỗi hộ đăng ký mảnh đất một lần vào PlotRegistryEntry (thuộc seller), tái sử dụng cho mọi listing sau; lúc tạo listing chỉ chọn từ registry và snapshot sang ProductPlot. Candidate spatial search + exact geometry hash + IoU/coverage kiểm soát reuse; overlap xuyên seller chỉ đặt plotReuseRisk=HIGH để OPERATOR review, không tự kết luận gian lận. declaredQuantity còn được đối chiếu trần sản lượng theo commodity/region để phát tín hiệu yield anomaly, không hard-reject."));
...
push(P("Level 1.5 và Level 2 là hai mô hình định danh khác nhau. Level 1.5 (Vinacontrol/Quatest) là actor thật trên nền tảng — dùng bảng inspector_signature riêng trong inspection_db với UNIQUE(reportId), KYC xác minh chứng chỉ hành nghề, session freshness riêng inspectionAuthMaxAgeSeconds = 1800s (thao tác thực địa cần thời gian, khác hành vi ký tức thời của buyer/seller). Level 2 (SGS/Bureau Veritas) không tích hợp đăng nhập — file-service poll hòm thư platform qua IMAP, phát file.ready/file.email_notice; inspection-service consume, hash đóng băng ngay khi nhận, OPERATOR/ADMIN xác nhận sau (human-confirm). Chọn tổ chức Level 2 giới hạn qua allowlist 3 nhóm để chặn thông đồng chọn tổ chức dễ dãi."));
push(P([runs("Ranh giới cross-service. ", { bold: true }), runs("contract_db signature chỉ có BUYER/SELLER. inspector_signature và inspection_report cùng thuộc inspection_db; mọi contractId/userId/fileId vẫn là plain UUID ở ranh giới service, không REFERENCES cross-database.", {})]));
...
push(P("Dịch vụ này gánh ba loại dữ liệu khác bản chất, không gộp chung logic: sổ khoá (là evidence ledger sở hữu quyết định, không phải projection chỉ-đọc — lockDurationDays snapshot cứng lúc tính, không recompute), điểm uy tín (view sống, tính lại được), và tham chiếu tín dụng (export cho bên thứ ba). Enforcement thực hiện ở tầng use-case chứ không ở Gateway: CreateListing, CreateOffer và sign() đều fail-closed khi không xác định được KYC/thẩm quyền/lock state. user-service là nơi enforce khoá thật dựa trên quyết định từ reputation-service."));
