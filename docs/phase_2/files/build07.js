...
push(P([runs("Cross-check tỉnh (giảm nhẹ lỗi convert VN2000→WGS84): ", { bold: true }), runs("cán bộ đo gốc bằng VN2000 rồi convert sang WGS84 trước khi xuất KML; nếu sai tham số kinh tuyến trục, toạ độ vẫn đúng DẠNG WGS84, vẫn trong bounding box Việt Nam, vẫn pass JTS (lệch tới ~81km nhưng là lỗi NỘI DUNG, không phải cú pháp). Đối chiếu hai nguồn độc lập: declaredProvince (seller chọn dropdown) vs tỉnh suy ra từ point-in-polygon (JTS + polygon Free-GIS-Data 34 tỉnh, cập nhật 08/07/2026 — đúng địa giới hiện hành NQ 202/2025). Lệch → provinceMismatchFlag=TRUE, cảnh báo UI, KHÔNG chặn insert.", {})]));
push(callout("Giới hạn cross-check tỉnh:", "chỉ bắt lỗi VÔ Ý đơn lẻ (một plot lệch khỏi tỉnh khai). Không bắt lỗi hệ thống (cả batch cùng lệch cùng hướng, nhất quán nội bộ) và không bắt fraud có chủ đích. Dùng Free-GIS-Data 34 tỉnh (08/07/2026 — nguồn mở sync GSO đúng địa giới hiện hành, gỡ false-positive sáp nhập của GADM 63 tỉnh cũ). Còn lại: nguồn cộng đồng, chưa phải bản đồ pháp lý chính thức của Cục Đo đạc — đủ cho cross-check non-blocking.", "note"));

push(H2("2.4.1 Geo-risk verification qua baseline rừng 2020 — ForTy/Natural Forests (06/07/2026; đổi từ NDVI tự tính sang baseline làm sẵn 08/07/2026)"));
push(P([runs("Cơ chế: ", { bold: true }), runs("ngay sau khi PlotRegistryEntry được tạo/update, hệ thống tự động (async, không chặn seller) query baseline rừng 2020 làm sẵn cho đúng EUDR (08/07/2026 — thay tự tính NDVI + tự đặt ngưỡng): ForTy 2020 (Forest Typology v1.0, Google Earth Engine — Class 5 = Tree Crops & Agroforestry phân loại thẳng là nông nghiệp EUDR, tách khỏi rừng tự nhiên) hoặc Natural Forests 2020 baseline (10m, 92% accuracy, thiết kế cho EUDR due diligence). Rơi vào class rừng tự nhiên 2020 → geoRiskLevel = HIGH_RISK; class cây trồng/nông nghiệp → LOW_RISK. Không còn ngưỡng NDVI thủ công. Không chặn giao dịch — chỉ dán nhãn cho buyer tự quyết định, đúng nguyên tắc neutral-party.", {})]));
...
push(P([runs("2 field mới, tách biệt rõ tiến trình vs kết quả: ", { bold: true }), runs("geoVerificationStatus (PENDING/CHECKED/UNVERIFIED) là trạng thái check đã chạy xong chưa; geoRiskLevel (LOW_RISK/HIGH_RISK/INCONCLUSIVE, nullable) là kết quả thật, chỉ có giá trị khi CHECKED. geoVerificationTimeoutHours (chốt 6 giờ, application.yml, tinh chỉnh được) — tính bằng GIỜ chứ không phải ngày, vì dữ liệu đã tồn tại sẵn, timeout chỉ hứng lỗi tạm thời (mây che, rate limit). Hết timeout mà chưa có kết quả → UNVERIFIED. CreateListing reject nếu registryEntryId đang PENDING (tránh race condition listing lên sàn trước khi biết risk).", {})]));
...
push(callout("Giới hạn đã biết (Known Limitation):", "Baseline 10m 92% accuracy là proxy tham khảo, KHÔNG phải xác minh EUDR pháp lý đầy đủ — không được trình bày với VICOFA/buyer như \"đã verify EUDR\". EXIF chỉ là lớp phòng thủ bổ sung — không giải quyết được GPS spoofing. (Điểm treo \"ngưỡng NDVI\" đã gỡ nhờ baseline ForTy/Natural Forests làm sẵn; geoVerificationTimeoutHours chốt 6 giờ, 13/07/2026 — không còn điểm treo số liệu.)", "note"));
...
uc("UC-P2", "AddPlotManually (fallback)", [
  ...
  ["Luồng chính", "Tạo/update PlotRegistryEntry (geometryType=POINT, source=MANUAL_PIN, originalKmlFileId=NULL); cùng cross-check tỉnh. accuracyMeters từ coords.accuracy; > ngưỡng 20m (chốt, application.yml) → cảnh báo UI, KHÔNG chặn cứng (thiết bị rẻ/trong nhà kính không đạt ngưỡng vẫn phải dùng được)"],
]);
...
push(callout("Cao su — nguồn tham chiếu quốc tế (L5, 08/07/2026):", "cao su CÓ nguồn giá quốc tế (anfin RSS3/TSR20, thitruonghanghoa SICOM/TOCOM, investing.com, VRA) — chủ động chọn giữ Admin nhập tay, KHÔNG ingest tự động ở scope hiện tại — ghi nhận là roadmap vì giá quốc tế tính JPY/kg hoặc USD/tấn cần chuẩn hoá đơn vị + tỷ giá real-time cho đúng 1 commodity (trade-off không đáng cho scope). Ghi nhận là enhancement path đã cân nhắc, không phải điểm mù. Điều giữ nguyên Admin — không có nguồn tương đương.", "note"));
