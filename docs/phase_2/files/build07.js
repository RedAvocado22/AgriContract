...
push(callout("Giới hạn, chưa đóng (06/07/2026):", "Baseline 10m 92% accuracy là proxy tham khảo, KHÔNG phải xác minh EUDR pháp lý đầy đủ — không được trình bày với VICOFA/buyer như \"đã verify EUDR\". Điểm treo \"ngưỡng NDVI chưa chốt\" đã GỠ nhờ chuyển sang baseline ForTy/Natural Forests làm sẵn (08/07/2026); còn treo: geoVerificationTimeoutHours placeholder 4-6h chưa vận hành thật. EXIF chỉ là lớp phòng thủ bổ sung — không giải quyết được GPS spoofing.", "note"));
...
push(table(
  ...
  [
    ...
    ["RUBBER", "Có nguồn tham chiếu quốc tế (anfin/thitruonghanghoa/investing/VRA) — xem ghi chú L5", "Admin nhập tay (không ingest quốc tế Phase 2 — trade-off đơn vị/tỷ giá)"],
...
push(callout("Cao su — nguồn tham chiếu quốc tế (L5, 08/07/2026):", "cao su CÓ nguồn giá quốc tế (anfin RSS3/TSR20, thitruonghanghoa SICOM/TOCOM, investing.com, VRA) — chủ động chọn giữ Admin nhập tay, KHÔNG ingest tự động Phase 2 vì giá quốc tế tính JPY/kg hoặc USD/tấn cần chuẩn hoá đơn vị + tỷ giá real-time cho đúng 1 commodity (trade-off không đáng cho scope). Ghi nhận là enhancement path đã cân nhắc, không phải điểm mù. Điều giữ nguyên Admin — không có nguồn tương đương.", "note"));
