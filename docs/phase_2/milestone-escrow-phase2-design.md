...
Bản thân việc **có** guardrail là câu trả lời cho câu hỏi hội đồng dễ hỏi: "làm sao chống buyer ép điều khoản?" — có số cụ thể defend được dễ hơn khoảng trống.
...
- Release ngay cho seller = `X15 × (1 − rate)` — dùng `X15`, **không phải** `batchAmount` (nếu `X15 < batchAmount` mà release theo `batchAmount` thì trả thừa, không đòi lại được).
- Giữ khoá phần còn lại = `batchAmount − X15×(1−rate)`, gồm 2 khối: `X15 × rate` (buffer của seller, chờ Level 2 xác nhận) + `batchAmount − X15` (phần thuộc buyer theo phán quyết 1.5 — **không** refund buyer sớm ở bước này, vì nếu Level 2 sau đó phán seller đáng nhận nhiều hơn thì đòi lại từ buyer cũng khó y hệt đòi seller).
- `EscrowMilestone.status = PROVISIONALLY_RELEASED` (state mới, thêm vào §2.3). Field `bufferAmount` (= `batchAmount − X15×(1−rate)`) snapshot lên `EscrowMilestone` lúc này — đây là snapshot phán quyết bất biến, **không** phải số dư phải đồng bộ tay với bank-service (không vi phạm nguyên tắc "escrow không giữ số tiền" ở `bank-service-phase2-design.md` §5, cùng logic với `lockDurationDays` bên reputation-service).
- Ledger: `RELEASE_TO_SELLER(X15×(1−rate))`. Không refund gì cho buyer ở bước này.
...
- Bù thêm cho seller = `max(0, min(X2,batchAmount) − X15×(1−rate))`, lấy từ phần đang giữ.
- Refund buyer = phần giữ còn lại sau khi bù seller.
- **Nếu `X2 < X15×(1−rate)`** (Level 2 phán thấp hơn số đã lỡ release ở Bước 1): seller đã nhận thừa, **không đòi lại được** — đây chính là rủi ro mà `rate` (buffer) sinh ra để hấp thụ; phần thiếu hụt do buyer chịu, đúng nguyên tắc "nghiêng về seller yếu thế hơn" đã dùng xuyên suốt (§6.2), không phải bất công ngẫu nhiên.
- Ledger: `RELEASE_TO_SELLER(bù thêm)` + `REFUND_TO_BUYER(còn lại)`.

**Cập nhật (06/07/2026) — chốt hạn cứng, đóng điểm treo về buffer vô thời hạn:** thêm `level2BufferTerminalDays` (đặt tạm 30 ngày, tính từ lúc `level2BufferWindowDays` hết hạn — tổng thời gian chờ tối đa ~37-44 ngày trước khi có điểm dừng cứng).

**Bước 3 — Terminal (hết `level2BufferTerminalDays`, report Level 2 không bao giờ về):**
- Phán quyết 1.5 thành chung thẩm. Seller nhận nốt **`X15 × rate`** (buffer của chính seller, không phải toàn bộ phần đang giữ). Buyer nhận **`batchAmount − X15`** (phần thuộc buyer theo 1.5, tách riêng khỏi buffer của seller — đây chính là chỗ bản cũ gộp nhầm thành "release nốt bufferAmount cho seller").
- Ledger: `RELEASE_TO_SELLER(X15×rate)` + `REFUND_TO_BUYER(batchAmount − X15)`.
...
**Điểm còn treo, chưa chốt (06/07/2026):** cả 3 số (`level2BufferWindowDays`, `level2SafetyBufferRate`, `level2BufferTerminalDays`) vẫn là placeholder chưa validate với đơn vị Level 2 thật cho đúng thị trường VN — giữ nguyên trạng thái honest, không đoán số để lấp.

**Cập nhật (08/07/2026) — `level2BufferWindowDays` neo theo benchmark có nguồn, thay số đoán thuần cảm tính:** đặt tạm 7-14 ngày (bản 06/07/2026) là đoán chưa có căn cứ. Research ngành `[REFERENCE]`: PSI kiểm hàng số lượng/đóng gói thường 24-48h; lab test chất lượng chuyên sâu (cupping cà phê, độ ẩm, phân tích cảm quan — đúng loại cho tranh chấp chất lượng nông sản) chuẩn SCA là **5 ngày làm việc kể từ khi nhận mẫu**; lab compliance test tổng quát 3-15 ngày làm việc. **Điều chỉnh khuyến nghị: neo `level2BufferWindowDays` vào 5-10 ngày làm việc** (5 ngày lab test SCA + buffer lịch hẹn/vận chuyển mẫu) thay vì 7-14 đoán thuần. Đây vẫn là benchmark quốc tế `[REFERENCE]`, chưa validate với đơn vị Level 2 thật cho thị trường VN (đã liên hệ NHL, chưa có phản hồi) — ghi rõ trong SDS là "neo theo benchmark lab test cà phê SCA, chưa validate thị trường VN", không phải số đo thực tế.
...
| RabbitMQ | `milestone.buyer_confirmed` | contract-service | notification-service, audit (**sửa 08/07/2026 — bỏ escrow-service**, xem ghi chú dưới) |
...
**Sửa (08/07/2026) — `milestone.buyer_confirmed` là event thừa, nguy cơ release tiền 2 lần:** Event Catalog bản trước liệt kê `escrow-service` là consumer của `milestone.buyer_confirmed` với vai trò "trigger release" — nhưng luồng release tiền thật chạy qua `milestone.settled` (mang `lockedAmount`/`actualAmount`, xem ngay dưới). Dưới Milestone Escrow, milestone settle không cần "buyer confirm" làm tín hiệu tiền tách rời nữa: buyer `CONFIRM_CLEAN` → milestone `SETTLED` → contract-service publish `milestone.settled`. Nếu escrow-service consume **cả hai** event cho cùng 1 hành động → release tiền 2 lần cho cùng milestone. Đây là di sản mô hình Phase 1 (nơi "buyer confirm" là sự kiện riêng) chưa dọn. **`milestone.settled` là nguồn trigger tiền DUY NHẤT** — `milestone.buyer_confirmed` giữ lại cho notification/audit (ghi nhận "buyer đã bấm xác nhận"), escrow-service tuyệt đối không nghe nó.
...
| Loại | Tên | Publisher | Consumer(s) |
|---|---|---|---|
| RabbitMQ | `contract.signed` | contract-service — **mới (06/07/2026, phát sinh từ session `analytics-service`); payload mở rộng (08/07/2026, phát hiện qua rà end-to-end)**. Bắn đúng 1 lần khi `Contract.transitionTo(SIGNED)` xảy ra — trigger cụ thể là bước 4 của `VerifyOtpAndSign` (`signature-phase2-design.md` §6, bước 5 nối tiếp cũng publish event này, tách biệt với việc push `signedContentHash` riêng cho audit-service). Payload: `{contractId, commodity, buyerId, sellerId, totalAmount, buyerDepositAmount, sellerDepositAmount, signedAt}`. `commodity` dùng chung enum `COFFEE / RICE / RUBBER / CASHEW` đã có tiền lệ ở `pricing-service-phase2-design.md` §2 — **cập nhật (08/07/2026, đồng bộ với Phase 1 Category redesign):** `Product.category` không còn là enum tiếng Việt tầng rộng mà là `categoryId` (FK) trỏ tới aggregate `Category` (admin/seller propose động). `commodity` giờ là **field trên chính `Category`** (`Category.commodity`, enum cứng 4 giá trị, admin gán bắt buộc lúc `approve()` category). contract-service đọc `Category.commodity` của category gắn với sản phẩm → publish thẳng, **không cần bảng mapping tra cứng nào nữa** (điểm map đã dời về lúc admin duyệt category, đúng người có thẩm quyền, 1 lần). Không có case rơi vào `NULL` — vì category chỉ dùng được khi đã `APPROVED`, mà `approve()` bắt buộc gán `commodity`, nên mọi category hợp lệ luôn có commodity. | analytics-service (populate `dim_contract`, `analytics-service-phase2-design.md` §2.1/§3.1), **escrow-service (mới, 08/07/2026 — trigger lock, xem ghi chú payload ngay dưới)** |
...
| `level2BufferWindowDays` (đặt tạm 5-10 ngày làm việc, sửa 08/07/2026 từ 7-14) | `application.yml` | **Mới (06/07/2026), điều chỉnh (08/07/2026):** neo theo benchmark lab test cà phê chuẩn SCA (5 ngày làm việc/mẫu) + buffer lịch hẹn/vận chuyển mẫu, thay vì đoán thuần 7-14 — vẫn `[REFERENCE]` quốc tế, chưa validate với đơn vị Level 2 thật cho thị trường VN (đã liên hệ NHL, chưa có phản hồi). Hết window mà chưa có report `CONFIRMED` → commission Level 1.5 fallback, settle tạm theo mechanics 3 bước (§3.2). |
...
Milestone Escrow — **đóng session, sẵn sàng đưa vào Architecture/SDS/TechnicalSpec chính thức**, ngoại trừ 1 phần **chưa đóng** vì phụ thuộc dữ liệu ngoài bàn thiết kế (không phải chưa quyết được): provisional settlement Level 2 — `level2BufferWindowDays` chốt 5-10 ngày (neo benchmark SCA), còn `level2SafetyBufferRate` (10-15%) và `level2BufferTerminalDays` (30 ngày) giữ placeholder honest — cần variance thật giữa Level 1.5 và Level 2 mới tinh chỉnh được, không đoán số để lấp. **Đã chốt hết các quyết định thiết kế:** Delta 2 chia phần-vượt/khối-lượng (B1), `graceDays` per-contract (A4), guardrail range (B3), `lockDurationDays` nặng hơn cho cancel-ở-0-milestone **áp dụng** (§6.1 — ký xong bỏ ngay là tín hiệu xấu nhất). **`contract.signed` mapping `commodity`:** đã đóng — đồng bộ với Phase 1 Category redesign, `commodity` đọc thẳng từ `Category.commodity` (admin gán lúc approve), không cần bảng tra cứng (§7.2).

---

*Design session: 02/07/2026 · Cập nhật: 04/07/2026 (Local Outbox, buyer timeout, buyerDepositRate release paths) · Cập nhật: 06/07/2026 (thêm event `contract.signed` cấp Contract + đăng ký analytics-service làm consumer của `contract.settled`/`contract.cancelled`, phát sinh từ rà soát chéo `analytics-service-phase2-design.md`; thêm provisional settlement + buffer cho Level 2 chậm report, §3.2/§7.1/§8; thêm chốt hạn cứng `level2BufferTerminalDays`; thêm `sellerDepositRate` optional thay quyết định "bỏ hẳn", §2.1/§2.3/§6.1/§6.2/§6.3 — chưa đóng, còn các điểm treo) · Cập nhật: 08/07/2026 (rà soát end-to-end: payload `contract.signed` mở rộng + event `escrow.deposit_locked` thêm lại — A1/A2; mechanics đầy đủ Provisional Settlement, sửa lỗi bufferAmount — A3; `expectedDeliveryDate`/`graceDays` + 2 timeout mới `SELLER_WEIGHED`/`IN_PROGRESS` — A4; Delta 2 chia tolerance thay vì seller gánh 100% — B1; bỏ escrow-service khỏi consumer `milestone.buyer_confirmed` — B2; guardrail range cho tolerance/threshold/penalty rate — B3; định nghĩa rõ SIGNED vs ACTIVE — B5) · Chưa code · Chưa đưa vào Architecture/SDS/TechnicalSpec chính thức.*
