...
| 8 | Contract terminal không còn tiền lock treo | Sau `contract.settled`/`terminated`/`SUPERSEDED`/`ACTIVATION_FAILED` (sửa tên event 19/07): tổng lock theo contract = 0 (reconciliation test) | bank §3.2, escrow |
...
| 11o | Một `remedyDecisionId` → đúng 1 bộ bank legs, ≤ 1 reputation lock (chặn double-consume) | Phát `remedy.finalized` rồi `contract.terminated` cùng breach (eventId khác nhau) → chỉ 1 bộ ledger legs (bank theo remedyDecisionId), reputation `remedy_decision_id UNIQUE` chặn lock thứ 2; replay `remedy.finalized` → 0 side effect mới | milestone-escrow §7.2; reputation §2.1; bank §2 (19/07 lần 2) |
...
*Design session: 18/07/2026 · Cập nhật 19/07/2026 (đợt 6 — 14 rows attribution/remedy/cap/funding/inspection-guard: 11g-11o P0 [9], 26e-26i P2 [5]; review lần 2 cùng ngày: row 8 đổi tên event, 26h đổi tên, +11o double-consume) · Chưa code · Test plan ưu tiên cao nhất trước khi implement.*
