...
|---|---|---|---|
| 1 | Event lặp không release/lock tiền hai lần | Publish cùng `eventId` nhiều lần → đúng 1 bộ ledger entry | bank §4 idempotency |
...
| 5 | Ledger lệch với record đã anchor phải bị phát hiện | Sửa 1 ledger entry lệch `settledAmount` → `LedgerAuditReconciliationJob` bắn `reconciliation_mismatch` | bank §5b.1 |
...
| 9 | Kill switch chặn toàn bộ dòng tiền | `system_lock ACTIVE` → mọi `bank.*_requested` fail có kiểm soát; unlock sai chữ ký/replay nonce → reject | bank §3.5.2-3.5.5 |
| 10 | Provisional Level 2 không release quá mức sàn khi buyer im lặng | Hết `disputeOptOutWindowDays` không phản hồi → release mức default sàn | milestone §3.2 Bước 0 |
...
| 11f | source_hash 3-way khớp, record_hash verify riêng | `Contract.signedContentHash` == `audit_record.source_hash` == hash email; sửa 1 trong 3 → reconcile bắt; record_hash KHÔNG so trực tiếp với signedContentHash | hash-chain §4.1 (18/07 r2) |
