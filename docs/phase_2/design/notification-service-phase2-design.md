...
| `notification.remedy_finalized_requested` / contract-service (**mới 19/07/2026**) | Buyer + Seller | Canonical remedy fields gồm `remedyDecisionId`, `attributionDecisionId`, nullable `breachCaseId`, `contractId`, `buyerId`, `sellerId`, `affectedMilestoneIds[]`, nullable final attribution/reason, eligibility flags và exact `remedyLegs`; command chỉ bổ sung `payload.recipients` cho routing |
