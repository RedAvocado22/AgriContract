...
|---|---|---|---|
| `remedy.finalized` (**canonical negative input**) | Negative | `contract-service` — bắn khi `AttributionDecision` được chốt, gồm `buyerId`, `sellerId`, `finalBreachingRole`, `breachReasonCode`, `decisionSource` và `remedyDecisionId` | **Gate cứng:** chỉ tạo `lock_entry` khi `reputationEligible = true`; map `finalBreachingRole` to `buyerId`/`sellerId` from the same event, never by lookup or role inference. Idempotency theo `remedyDecisionId`; allegation (`breach.reported`) không phải input. |
...
| `contract.settled` | Positive | `contract-service` (`ContractSettledEvent`) | Guard `Contract.settle()` chạy từ `ACTIVE`; reputation ghi immutable completion fact. Escrow không consume event này for money; normal settlement legs arrive through `remedy.finalized`. |
| `milestone.dispute_resolved` | Tín hiệu hành vi (flag-abuse) | `contract-service` — payload gồm `contractId`, `milestoneId`, `flaggedByUserId`, `resolutionFavors` | Consumer inserts `dispute_outcome_fact` directly from canonical IDs; no participant lookup. |
...
|---|---|---|---|
| `milestone.dispute_resolved` | Tín hiệu hành vi | `contract-service` — payload `{contractId, milestoneId, flaggedBy: BUYER, flaggedByUserId, resolutionFavors: BUYER\|SELLER}` | `reputation-service` đếm tỷ lệ `resolutionFavors = SELLER` trên tổng số lần buyer đó từng `FLAG_ISSUE`; IDs đã canonical nên không cần lookup |
...
6. **Mới (10/07/2026) — tầng batch bắt structuring chậm + trạng thái `ELEVATED_RISK` (Enhanced Due Diligence):** 2 tầng ở mục 1-2 chạy realtime và đều chỉ với tay tới **giao dịch kế tiếp** — không bắt được kẻ rải mỏng nhiều hợp đồng ~490 triệu (dưới ngưỡng tuyệt đối) giãn cách nhiều tháng (ngoài cửa sổ tương đối). Đây chính là gap ghi ở §9. Đóng bằng tầng batch trên data warehouse: `analytics-service.AmlPatternScanJob` (`analytics-service-phase2-design.md` §3.5) quét cụm near-threshold theo cặp buyer-seller (band `[475tr, 500tr)`, min 5 hợp đồng, lookback 90 ngày) và publish `analytics.structuring_pattern_detected`.
   ...
   **`reputation-service` consume event này → set cặp buyer-seller sang trạng thái `ELEVATED_RISK`.** Đây là điểm khác biệt cốt lõi so với cơ chế hold ở mục 2, phải nắm rõ để không implement nhầm:
   ...
   - **Phase 2 freeze: `ELEVATED_RISK` là projection-only.** Reputation-service persist và expose trạng thái cặp cho Admin/analytics; Phase 2 **không** chặn auto-settlement và không route sang synchronous review, vì contract/milestone design chưa có settlement guard/review use case hoặc transport/error contract tương ứng. Claim enforcement cũ được bỏ khỏi Phase 2 thay vì ngầm phát minh workflow mới.
   - Gate composite realtime ở mục 2 giữ nguyên cho các giao dịch lẻ đã có contract. Pair-state enforcement là Future Work chỉ được bật sau một design freeze riêng xác định service owner, settlement guard, review state/use case và request/error contract.
...
Fix + dead-path cleanup đã chốt tại `milestone-escrow-phase2-design.md` §3.1 (guard sửa để chạy từ `ACTIVE`). Reputation consumes `contract.settled` only for positive completion facts; deposit release is represented by `remedy.finalized` legs. `§3` của file này đã cập nhật theo.
...
**Chốt bổ sung (10/07/2026; scope freeze 20/07/2026) — tầng batch AML + `ELEVATED_RISK` (§8 mục 6, §9):** `analytics-service.AmlPatternScanJob` (batch 90 ngày, cụm near-threshold) publish `analytics.structuring_pattern_detected` → `reputation-service` consume → set cặp `ELEVATED_RISK` bền, có clear-path + expiry (`elevatedRiskReviewMonths` = 6 tháng) và audit. Trong Phase 2 đây là **projection-only**; không chặn settlement cho tới khi có settlement-review contract được freeze ở phase sau. Nghĩa vụ báo cáo cơ quan tách sang `bank-service` (consumer thứ 2 của cùng event). Residual còn lại (vài mảnh đầu chưa đủ cụm + one-shot nhỏ lẻ thật) là giới hạn toán học của pattern-based detection, ghi honest ở §9.
