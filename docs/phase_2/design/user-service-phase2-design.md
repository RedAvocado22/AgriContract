...
- `reputation.locked {eventId, userId, lockedUntil, lockRevision, occurredAt}` → apply nếu `lockRevision > last_lock_revision` (atomic, cùng UPDATE), set `lockedUntil = max(current, incoming)` + `last_lock_revision = lockRevision`.
- `reputation.unlocked {eventId, userId, effectiveLockedUntil, occurredAt}` (**sửa 17/07/2026**) → set `lockedUntil = effectiveLockedUntil` (timestamp nếu user còn lock khác đang hiệu lực, `NULL` nếu hết khoá). Không tự "clear": user có thể có **nhiều lock chồng nhau** (cancel 2 hợp đồng → 2 `lock_entry`), unlock sớm 1 cái không có nghĩa hết khoá — reputation-service (source of truth, giữ toàn bộ `lock_entry`) tính lại effective lock rồi gửi kết quả; user-service chỉ set theo, không suy luận. **Ordering bằng `lockRevision` persist (sửa 18/07/2026):** reputation-service cấp revision tăng dần per user (đếm số event lock/unlock đã phát cho user đó); user_profiles lưu `last_lock_revision`, chỉ apply khi incoming lớn hơn — atomic trong cùng UPDATE. `occurredAt` giữ lại cho audit/debug nhưng KHÔNG dùng làm ordering: nó không được persist trước đây (restart xong không còn gì để so — idempotency log chỉ chặn đúng eventId đã thấy, không chặn event CŨ có eventId khác tới muộn) và timestamp dính clock skew/tie.
...
| Tạo listing | `VERIFIED` và authorization còn hạn | Không cho nếu đang khoá | **Fail-closed (sửa 18/07/2026, round 2):** user-service down → 503, không tạo. Fail-open cũ tự mâu thuẫn — lúc dependency chết caller không biết CẢ 3 điều kiện mà vẫn cho qua, người chưa KYC/đang khoá tạo được listing đúng lúc đó. Listing không khẩn cấp tới mức hy sinh policy; giờ MỌI gate eligibility đều fail-closed, nhất quán với `sign()` |
| Tạo offer | `VERIFIED` và authorization còn hạn | Không cho nếu đang khoá | Cùng chính sách create listing — fail-closed (18/07/2026) |
...
### 4.2 Admin KYC API — qua Gateway, role `ADMIN|OPERATOR` (sửa 18/07/2026 — OPERATOR duyệt/reject theo governance §5, ADMIN là fallback; matrix quyền ở `data-governance-phase2-design.md` §5.4)
...
| `reputation.locked` | reputation-service | user-service | `{eventId, userId, lockedUntil, reasonCode, lockRevision, occurredAt}`; không mang email |
| `reputation.unlocked` | reputation-service | user-service | `{eventId, userId, effectiveLockedUntil (nullable), reasonCode, lockRevision, occurredAt}`; **sửa 17/07/2026** — mang effective lock còn lại do reputation tính lại (đóng edge case nhiều lock chồng nhau); không mang email |
...
ALTER TABLE user_profiles
  MODIFY role ENUM('SELLER','BUYER','ADMIN','INSPECTOR','OPERATOR') NOT NULL,  -- OPERATOR: 18/07/2026, governance §5
  ADD COLUMN authorization_expires_at DATETIME(6) NULL,
  ADD COLUMN locked_until DATETIME(6) NULL,
  ADD COLUMN verified_by_actor_id VARCHAR(255) NULL,  -- SỬA 18/07/2026: đổi tên từ verified_by_admin_id —
                                                      -- OPERATOR cũng duyệt KYC (governance §5), tên cũ nói dối
  ADD COLUMN last_lock_revision BIGINT NOT NULL DEFAULT 0,  -- ordering lock events, §2.2 (18/07/2026)
...
- Event lock/unlock xử lý idempotent; ordering theo `lockRevision` **persist trong `last_lock_revision`** (**sửa 18/07/2026** — `occurredAt` 17/07 chưa đủ: không persist thì restart xong không còn gì để so, và timestamp dính clock skew). Unlock không clear mù — set theo `effectiveLockedUntil` (§2.2).
...
- Tự động đối soát giấy phép/đăng ký doanh nghiệp với cơ sở dữ liệu Nhà nước ngoài scope; Operator/Admin duyệt thủ công.
...
**Chốt:** user-service sở hữu profile/KYC/authorization và cache lock; Keycloak sở hữu credential; contract-service sở hữu Signature. Public/internal DTO tách hẳn để đóng KI-3. Enforcement ở use-case, không ở Gateway; `sign()` fail-closed. Events KYC và lock đủ dữ liệu cho notification nhưng không làm notification gọi sync ngược. **Cập nhật 17/07/2026:** `reputation.unlocked` mang `effectiveLockedUntil` do reputation-service tính lại từ các `lock_entry` còn hiệu lực — đóng edge case user có nhiều lock chồng nhau bị mở khoá nhầm khi unlock sớm 1 lock; cả 2 event lock/unlock thêm `occurredAt` cho ordering (§2.2, §5, §7).
