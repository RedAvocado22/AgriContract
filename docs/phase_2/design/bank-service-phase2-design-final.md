...
- **Deposit cấp Contract** (§3.2) — `contract.settled` trả cọc về đúng chủ theo normal settlement; mọi outcome tiền khi termination được mô tả bằng `remedy.finalized.remedyLegs` với `milestoneId = NULL`. `contract.terminated` chỉ là event trạng thái/audit/analytics/notification, không kích ledger; remedy type hiện hành là `DEPOSIT_FORFEITURE`/`CONTRACTUAL_PENALTY`/`DAMAGES_COMPENSATION`.
