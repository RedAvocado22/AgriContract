...
**⟢ Lưu ý cho team code (cập nhật 17/07/2026):** bảng `signature` của `contract_db` **chỉ** chứa nhánh `BUYER`/`SELLER` — phương án cũ "mở rộng enum + `report_id` cho INSPECTOR" đã superseded vì tạo cross-service write không có đường thực thi (inspector thao tác ở inspection-service, không được ghi `contract_db`). Nhánh INSPECTOR giờ nằm ở bảng riêng `inspector_signature` trong DB của inspection-service (owner: `inspection-phase2-design.md` §2.1/§5) — chung cơ chế step-up/session-freshness, không chung bảng vật lý. Migrate bảng `signature` theo đúng file này là đủ.
...
VerifyOtpAndSign(otpId, contractId, otpCode):   // sửa 18/07/2026 — verify theo otpId,
  ...
  0. Load challenge = signature_otp[otpId]; không tồn tại/đã verified → REJECT.
     Bind challenge — đóng gap 2 HTTP request độc lập:
       ...
       challenge.signer_role == role của caller trong contract này (theo Contract, không theo request)
  ...
  5. Publish `notification.contract_anchor_requested` cho notification-service,
     ...
     (Sửa 17/07/2026: KHÔNG còn "push hash vào audit-service" như 1 đường riêng chưa
  ...
  6. Publish `contract.signed` (RabbitMQ, domain event — mới 06/07/2026; payload mở
     ...
     (source_type CONTRACT_SIGNED). Consumer: escrow-service, analytics-service,
     audit-service. Payload + consumer đầy đủ — chi tiết catalog ở
...
CREATE TABLE signature_otp (
    ...
    auth_time          TIMESTAMP NOT NULL,       -- SỬA 18/07/2026: challenge persistence — step-up authTime
                                                -- từ InitiateSign; VerifyOtpAndSign là HTTP request độc lập,
                                                -- không có chỗ nào khác lấy lại giá trị này
    signed_content_hash VARCHAR(64) NOT NULL,    -- SỬA 18/07/2026: hash terms tại thời điểm phát OTP —
                                                -- verify so với terms hiện tại, lệch = terms đã đổi
    otp_hash          VARCHAR(64) NOT NULL,     -- SỬA 18/07/2026: HMAC-SHA256(otp, serverPepper) — pepper từ
                                                -- secret/env, KHÔNG SHA-256 trần: chỉ 10^6 mã, ai đọc được DB
                                                -- brute-force xong trong tích tắc
...
-- user_db: migration authorization_expires_at do user-service sở hữu — bảng thật là
-- `user_profiles` (KHÔNG phải `app_user` như bản cũ ghi; sửa 17/07/2026, defer sang
-- user-service-phase2-design.md §6 làm source of truth cho user_db).
...
- **Contract amendment sau khi ký (18/07/2026):** không hỗ trợ Phase 2 — `ContractTerms` đóng băng + hash là nền evidence model; đổi điều khoản = huỷ theo điều khoản hoặc ký hợp đồng thay thế. Roadmap `ContractVersion` chain: milestone-escrow §8b.
