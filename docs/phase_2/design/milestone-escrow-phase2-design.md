...
**Listing-version guard tại create (chốt 23/07/2026):** `ListingResponse` trả `listingVersionToken` opaque; Phase 2 sinh token từ canonical UTC representation của `Listing.updatedAt`. Buyer client giữ đúng token của listing đã hiển thị và gửi nó trong `CreateContractRequest`; canonical partial-fill create không chấp nhận missing/null. Product-service so token trong chính atomic reservation transaction; mismatch phải reject `409 LISTING_VERSION_MISMATCH` với thông điệp "listing đã thay đổi, xem lại", không âm thầm reserve/snapshot dữ liệu mới. Sau khi pass, immutability/hash behavior hiện có giữ nguyên.

### 2.1d Listing partial-fill reservation và contract create ordering (chốt 23/07/2026)

`requestedQuantity` không phải field độc lập do client gửi; contract-service luôn derive `Σ milestoneSchedule[].committedQuantity`. Product-service sở hữu `quantityAvailable` và reservation aggregate; contract-service chỉ persist `reservedQuantity = requestedQuantity` để đối chiếu về sau, không giữ hay cập nhật inventory counter.

Canonical create flow của `POST /api/v1/contracts`:

1. Validate terms/schema, derive `requestedQuantity`, generate `contractId` trước khi ghi Contract.
2. Gọi sync internal product reservation với `{contractId, listingId, requestedQuantity, listingVersionToken}`. Product-service atomically validate `minOrderQuantity <= requestedQuantity <= quantityAvailable`, compare token, insert reservation unique theo `contractId` và decrement tồn trong một transaction.
3. Chỉ khi reserve thành công mới persist Contract `OFFERED`, `reservedQuantity`, milestones/snapshot và outbox `contract.created` trong cùng local transaction. `contract.created` xác nhận reservation đã gắn với contract, không decrement lại.
4. Nếu local persistence fail sau reserve, contract-service bắt buộc gọi compensation release idempotent theo `contractId`; retry cùng command tới khi nhận success/no-op. Không được để một contract `OFFERED` tồn tại nếu chưa reserve thành công.

Không đủ hàng, dưới minimum hoặc listing legacy `quantityAvailable = NULL` trả `409 INSUFFICIENT_LISTING_QUANTITY`; client reload listing và không blind retry. `LISTING_VERSION_MISMATCH` vẫn áp khi token stale nhưng tồn còn đủ. Replay cùng `contractId`/payload trả cùng reservation không decrement lần hai; payload khác với cùng `contractId` là state conflict.

Contract lifecycle publish durable `contract.created`, `contract.withdrawn`, `contract.signed`, `contract.activation_failed`, `contract.activated`, đều mang `contractId`, `listingId`, `reservedQuantity` và lifecycle timestamp. Product-service consume idempotent theo envelope `eventId`: withdraw release pre-sign; signed chỉ đánh dấu signed-but-releasable; activation failure release; activated mới finalize `COMMITTED`. Các event inventory này không thay `contract.terminated`, `remedy.finalized`, attribution, conservation hay ledger terminal guard hiện có.

**F18 — sign guard:** ngay trước transition `SIGNED`, contract-service recompute `Σ milestoneSchedule[].committedQuantity` từ terms hiện tại và require bằng đúng persisted `reservedQuantity`. Mismatch fail closed `409`, không publish `contract.signed`, không kích escrow. Đúng tổng mới cho qua; `SIGNED` vẫn releasable và chỉ `ACTIVE`/`contract.activated` là inventory commit point.
...
| Loại | Tên | Publisher | Consumer(s) |
|---|---|---|---|
| RabbitMQ | `contract.created` | contract-service — publish từ cùng local transaction/outbox persist `OFFERED`; payload `{contractId, listingId, reservedQuantity, createdAt}`. Chỉ confirm synchronous reservation đã gắn với contract; không decrement inventory lần hai. | product-service |
| RabbitMQ | `contract.withdrawn` | contract-service — publish khi pre-sign offer commit `WITHDRAWN`; payload `{contractId, listingId, reservedQuantity, withdrawnAt}`. Event inventory chuyên biệt này tồn tại song song với lifecycle observation `contract.terminated(WITHDRAW_OFFER)`. | product-service (release idempotent) |
| RabbitMQ | `contract.signed` | contract-service — **mới (06/07/2026, payload inventory mở rộng 23/07/2026)**. Bắn đúng 1 lần khi `Contract.transitionTo(SIGNED)` xảy ra sau F18. Payload: `{contractId, listingId, reservedQuantity, commodity, buyerId, sellerId, totalAmount, buyerDepositAmount, sellerDepositAmount, signedContentHash, signedAt}`. `commodity` là enum cứng `COFFEE / RICE / RUBBER / CASHEW`; mapping owner ở `product-phase2-design.md` §9. | product-service (mark signed-but-releasable), analytics-service, escrow-service, audit-service |
| RabbitMQ | `contract.activation_failed` | contract-service — publish khi terminal `ACTIVATION_FAILED` đã qua refund/zero-lock guard; payload `{contractId, listingId, reservedQuantity, activationFailedAt}`. | product-service (release idempotent) |
| RabbitMQ | `contract.activated` | contract-service — publish sau khi `escrow.deposit_locked` làm transition `SIGNED -> ACTIVE`; payload `{contractId, listingId, reservedQuantity, activatedAt}`. Đây là inventory commit point. | product-service (finalize `COMMITTED`) |
...
**Sửa (08/07/2026; inventory sync 23/07/2026) — event kích hoạt ACTIVE:** `escrow.deposit_locked` vẫn là tín hiệu escrow→contract duy nhất để transition `ACTIVE` và để escrow-service nối tiếp lock milestone đầu. Sau khi commit transition, contract-service publish `contract.activated` qua outbox để product-service finalize reservation; event này không round-trip ngược để điều khiển escrow và không thay money flow.
...
- Listing concurrency/inventory: canonical create requires non-null token; reserve 40 + 40 trên listing 100 thành công và tồn `100 -> 60 -> 20`; reserve 30 tiếp theo trả `409 INSUFFICIENT_LISTING_QUANTITY`. Hai request tranh phần tồn cuối chỉ một conditional update thắng. Same `contractId` replay không decrement lần hai; legacy-null fail closed tới edit/republish.
- Reservation lifecycle: withdraw pre-sign và activation failure sau sign restore đúng lượng một lần; seller-closed/paused listing không tự mở khi restore; `contract.activated` mới finalize committed.
...
- Signature quantity: F18 mismatch giữa `Σ committedQuantity` và `reservedQuantity` chặn `SIGNED`; đúng tổng mới publish `contract.signed`.
...
Milestone Escrow — **ĐÓNG SESSION HOÀN TOÀN, sẵn sàng đưa vào Architecture/SDS/TechnicalSpec chính thức.** Provisional settlement Level 2: cả 3 số cấu hình (`level2BufferWindowDays` = 10 ngày, `level2SafetyBufferRate` = 15%, `level2BufferTerminalDays` = 30 ngày) đã chốt theo benchmark ngành ở §8, cấu hình `application.yml` tinh chỉnh được khi có dữ liệu vận hành thật. **Đã chốt hết các quyết định thiết kế:** Delta 2 chia phần-vượt/khối-lượng (B1), `graceDays` per-contract (A4), guardrail range (B3), `lockDurationDays` nặng hơn cho cancel-ở-0-milestone **áp dụng** (§6.1 — ký xong bỏ ngay là tín hiệu xấu nhất). **`contract.signed` mapping `commodity`:** đã đóng — `commodity` đọc thẳng từ `Category.commodity`; cơ chế chốt ở `product-phase2-design.md` §9 (owner). **Cập nhật 23/07/2026:** partial-fill reservation/inventory thuộc product-service; reserve trước `OFFERED`, F18 trước `SIGNED`, `ACTIVE`/`contract.activated` mới commit; `contract.created`/`withdrawn`/`activation_failed`/`activated` là lifecycle signals cho product-service.
