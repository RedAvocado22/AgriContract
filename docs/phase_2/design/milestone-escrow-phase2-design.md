...
- **Contract amendment (18/07/2026):** Phase 2 KHÔNG hỗ trợ sửa đổi sau khi hợp đồng đã ký — `ContractTerms` đóng băng + hash là nền của toàn bộ evidence model. Hai bên muốn đổi điều khoản: huỷ theo điều khoản (§6) hoặc ký hợp đồng thay thế (OTP flow rẻ, ký lại nhanh). Roadmap production: `ContractVersion` chain — amendment v2 mang `previousVersionHash` nối với bản trước, cần đủ 2 chữ ký buyer + seller như bản gốc, và **không được** sửa milestone đã `SETTLED` hay thay đổi tiền đã release; cần design session riêng (đụng signature flow, hash chain, recalc escrow), không nhét vào Phase 2.
