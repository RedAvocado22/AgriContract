...
|---|---|---|---|
| `WITHDRAW_OFFER` | Trước khi đủ 2 chữ ký | Không | Kết thúc negotiation, không tiền |
...
| `TERMINATION_FOR_BREACH` | Sau khi `BreachCase` kết luận `finalBreachingRole` | Theo `LegalProfile` + attribution | Pro-rata phần chưa `SETTLED`; seize/lock bên có lỗi (§6.4) |
...
- Seller quá `expectedDeliveryDate + graceDays` chưa `SELLER_WEIGHED`, không claim FM trong window (§3.2) → seller breach về thời hạn.
- Một bên tuyên bố dứt khoát bằng hành động sẽ không thực hiện (explicit repudiation).
...
*(Áp dụng cho `TERMINATION_FOR_BREACH` sau khi `BreachCase` §6.4 kết luận `finalBreachingRole`. Cơ chế tiền dưới đây giữ nguyên; điểm khác duy nhất so với bản cũ: **trigger không còn là `initiatedBy` mà là `finalBreachingRole`** — máy không tự suy người phá kèo từ người bấm nút.)*
...
Event `contract.terminated` với `terminationType = MUTUAL_REPLACEMENT` + `supersededByContractId`; hợp đồng mới phát `contract.signed` như bình thường.
...
|---|---|---|---|
| Hợp đồng hoàn tất — mọi milestone `SETTLED` | `contract.settled` (`ContractSettledEvent`, có sẵn Phase 1, cần guard fix §3.1) | `REFUND_TO_BUYER` | `RELEASE_TO_SELLER` |
| `MUTUAL_TERMINATION` / `MUTUAL_REPLACEMENT` (§6.5/§6.6) | `contract.terminated` (`terminationType`, `finalBreachingRole=NULL`) | `REFUND_TO_BUYER` | `RELEASE_TO_SELLER` (refund về chủ, không seize) |
| `TERMINATION_FOR_BREACH`, `finalBreachingRole=SELLER` (§6.3.1) | `contract.terminated` (`finalBreachingRole=SELLER`) | `REFUND_TO_BUYER` — độc lập penalty debt | `DEPOSIT_FORFEITURE` (nếu có khoá — offset vào penalty debt, §6.3.1 Case A) |
| `TERMINATION_FOR_BREACH`, `finalBreachingRole=BUYER` (§6.3.2) | `contract.terminated` (`finalBreachingRole=BUYER`) | `DEPOSIT_FORFEITURE` (chuyển cho seller) | `RELEASE_TO_SELLER` (seller không phá kèo) |
| `ACTIVATION_FAILURE` (§3.1, §6b) | `escrow.deposit_lock_failed` / funding-fail (§6b) | `REFUND_TO_BUYER` mọi leg đã lock | `REFUND_TO_BUYER` mọi leg đã lock |
...
`contract.terminated` — publisher `contract-service`, bắn 1 lần mỗi termination. **Phân vai event (sửa 19/07 lần 2 — chặn double-consume):** `remedy.finalized` là nguồn **DUY NHẤT** cho mọi hậu quả tiền (escrow→bank legs) và reputation; `contract.terminated` chỉ mang **trạng thái** cho audit + analytics + notification, KHÔNG kích seize/refund/lock nào — trước đó cả 2 event cùng kích chế tài ở escrow lẫn reputation, `sourceEventId` khác nhau nên idempotency không chặn được → nguy cơ phạt/khoá **hai lần** cho cùng một breach. Payload mở rộng (điểm 5): `{contractId, terminationType, requestedBy, finalBreachingRole?, breachReasonCode?, remedyDecisionId?, breachCaseId?, affectedMilestoneIds[], supersededByContractId?, replacesContractId?}` — `breachReasonCode` ở đây phục vụ analytics/audit/UI, KHÔNG dùng phát chế tài. Chi tiết catalog — §7.2.
...
| RabbitMQ | `milestone.cancelled_with_penalty` | contract-service — **sửa 19/07/2026: CHỈ bắn sau khi attribution xác nhận** (`remedy.finalized` với `finalBreachingRole` + reason đủ điều kiện penalty — §6.4; Delta 1 vượt threshold không còn tự bắn, §4 nhánh 2) | escrow-service (seize `batchAmount` lock theo remedyLegs), analytics-service (`fact_milestone_performance`), audit-service. **Reputation-service KHÔNG còn consume trực tiếp (19/07)** — nó consume `remedy.finalized` (reputation §3) |
...
| RabbitMQ | `contract.terminated` (mới, 19/07/2026 — **thay `contract.cancelled`**; phân vai sửa 19/07 lần 2) | contract-service — bắn 1 lần mỗi termination. Payload: `{contractId, terminationType: MUTUAL_TERMINATION\|MUTUAL_REPLACEMENT\|TERMINATION_FOR_BREACH\|TERMINATION_FOR_FORCE_MAJEURE\|ACTIVATION_FAILURE, requestedBy, finalBreachingRole?, breachReasonCode?, remedyDecisionId?, breachCaseId?, affectedMilestoneIds[], supersededByContractId?, replacesContractId?}` | **CHỈ state/quan sát:** audit-service (`source_type = CONTRACT_TERMINATED`), analytics-service (`fact_contract_termination`), notification (qua command §6.7). **KHÔNG escrow, KHÔNG reputation** — mọi hậu quả tiền/reputation đi qua `remedy.finalized` duy nhất (chặn double-consume; sourceEventId khác nhau không cứu được nếu 2 event cùng kích chế tài) |
...
| RabbitMQ | `remedy.finalized` (mới, 19/07/2026; canonical hoá 19/07 lần 2) | contract-service — **nguồn DUY NHẤT cho tiền + reputation**, bắn cho MỌI termination/chế tài: Rổ B khi `BreachCase` `RESOLVED`; Rổ A không dựng case — `breachCaseId = null`, `decisionSource = SYSTEM`; mutual/FM/supersede/technical — `finalBreachingRole = null`, remedyLegs toàn refund/release. Payload: `{remedyDecisionId, breachCaseId?, contractId, milestoneId?, finalBreachingRole?, breachReasonCode?, decisionSource: ADMIN\|INSPECTOR_L1_5\|INSPECTOR_L2\|MUTUAL\|SYSTEM, penaltyEligible: bool, reputationEligible: bool, remedyLegs:[{remedyType, fundType, amount, role}]}`. **2 flag eligibility (điểm 7 review):** consumer KHÔNG tự suy chế tài từ `breachReasonCode` — contract-service (remedy calculator + LegalProfile/phán quyết) quyết `penaltyEligible` (được áp CONTRACTUAL_PENALTY?) và `reputationEligible` (được lock?) rồi phát; vd `PRODUCTION_SHOCK_NON_FM`: penaltyEligible theo phán quyết, reputationEligible = false; `SIDE_SELLING`: cả hai = true. **Invariant:** một `remedyDecisionId` sinh đúng MỘT bộ bank legs và TỐI ĐA MỘT reputation lock (matrix 11o) | escrow-service (thực thi `remedyLegs` — chỉ RemedyDecision final), reputation-service (lock **chỉ khi** `reputationEligible = true`, idempotent theo `remedyDecisionId`), audit-service (`source_type = REMEDY_FINALIZED`) |
...
- **`buyerDepositRate` — 3 đường release/seize chính thức hoá** qua 2 event cấp Contract mới thêm vào catalog: `contract.settled` (event có sẵn, thêm consumer escrow-service) và `contract.cancelled` (event hoàn toàn mới) (§6.3, §7.2). *(19/07/2026: `contract.cancelled` nay là `contract.terminated`, mục nay là §6.7 — xem chốt đợt 6.)*
...
- **Cần sync sang (đã sync xong cùng ngày, gồm cả review lần 2):** bank-service (remedyType enum + fundType + remedyDecisionId), reputation-service (input DUY NHẤT `remedy.finalized`, đọc `reputationEligible`, `remedy_decision_id` UNIQUE), analytics (`fact_contract_termination` đọc từ `contract.terminated`), hash-chain (source_type `BREACH_REPORTED`/`REMEDY_FINALIZED`/`CONTRACT_TERMINATED`), notification (4 command), verification-matrix (53 rows). Còn treo: Doc 02 §3.5/§7 + SDS/Architecture regenerate.
