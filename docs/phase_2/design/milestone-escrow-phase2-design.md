...
| `breachReasonCode` | §6.4.1 |
| `evidenceRefs` | fileIds / timeout metadata (deadline nào, event vắng mặt nào) |

Luồng thống nhất: `AttributionDecision` → remedy calculator (§6.7) sinh `RemedyDecision` (`remedyDecisionId`, legs, 2 flag eligibility) → phát `remedy.finalized`. Rổ B: BreachCase `RESOLVED` tạo AttributionDecision với `breachCaseId`. Rổ A: timeout job tạo thẳng với `breachCaseId = NULL`, `decisionSource = SYSTEM`, evidenceRefs = metadata timeout. Một khái niệm, hai cửa vào, một cửa ra.
