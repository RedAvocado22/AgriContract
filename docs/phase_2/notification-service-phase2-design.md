...
| `notification.contract_cancelled_requested` / contract-service | Buyer + Seller | initiatedBy, cọc/nhánh xử lý |
...
ALTER TABLE notification_logs
  ...
  ADD COLUMN notification_type VARCHAR(100) NOT NULL,
  ADD COLUMN recipient_email VARCHAR(255) NOT NULL,
  ADD COLUMN template_version VARCHAR(50) NOT NULL,
...
Khi migrate, cột Phase 1 `user_id` đổi semantics thành `recipient_user_id` hoặc được giữ tạm rồi backfill; migration implementation phải không làm mất log cũ.
...
- In-app notification/WebSocket là enhancement; Phase 2 bắt buộc email + persisted log.
